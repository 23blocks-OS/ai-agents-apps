#!/usr/bin/env node
/**
 * Quick test script - extracts the HTML UI and opens in browser
 *
 * Usage:
 *   BLOCKS_API_URL=https://your-api.com BLOCKS_API_KEY=your-key node test-ui.js
 */

const API_URL = process.env.BLOCKS_API_URL || '';  // Forms API
const AUTH_URL = process.env.BLOCKS_AUTH_URL || '';  // Gateway/Auth API
const API_KEY = process.env.BLOCKS_API_KEY || '';

if (!process.env.BLOCKS_API_URL || !process.env.BLOCKS_AUTH_URL || !process.env.BLOCKS_API_KEY) {
  console.log('⚠️  Missing env vars. Set BLOCKS_API_URL, BLOCKS_AUTH_URL, and BLOCKS_API_KEY.\n');
}

// Generate the HTML (same as server does)
...
<head>
  ...
  <script>
    ...
    window.__BLOCKS_AUTH_URL__ = "${AUTH_URL}";
...
<body>
  ...
  <script type="module">
    ...
    const AUTH_URL = window.__BLOCKS_AUTH_URL__;
    ...
    function AuthProvider({ children }) {
      ...
      const login = useCallback(async (email, password) => {
        ...
        try {
          const res = await fetch(AUTH_URL + '/auth/sign_in', {
            method: 'POST',
          ...
          const user = data.data?.attributes || { email };
    ...
    function Dashboard() {
      ...
      const fetchLeads = async (formId, formName) => {
        ...
        try {
          const res = await fetch(API_URL + '/landing_forms?form_id=' + formId, {
            headers: { 'AppId': API_KEY, 'Authorization': 'Bearer ' + token },
          });
          if (res.status === 401) { logout(); return; }
          const data = await res.json();
